...
if (sessionUserData) {
    ...
    chapter = userInfo.completedChapters;
    sectionId = userInfo.sectionNumber;
    lesson = userInfo.currentLesson;
...
const resetBottomRow = () => {
    document.getElementById('skip-span').textContent = "Пропустить";
    ...
    document.getElementById('continue-button').textContent = 'Проверить'
...
let checkButton = (id) => {
    ...
    if (localStorage.getItem('correctIndex') == window.selectedOption) {
        ...
        if (clickCount == 0) {
            ...
            progressValue = (questionCount-1) * 25;
    ...
    document.getElementById(id).classList.toggle('clicked');
    setTimeout(() => document.getElementById(id).classList.toggle('clicked'), 300);
    document.getElementById('continue-button').textContent = 'Продолжить'
    clickCount++;
...
let questionLoad = () => {
    fetch(`https://webapiavzag.onrender.com/api/Question?sectionId=${sectionId}&chapter=${chapter}&lesson=${lesson}`)
        ...
        .then(data => {
            ...
            // СТАЛО:
            let challenge = data[questionCount-1]; // Берём первый (и единственный) challenge
            
            if (questionCount < 5) {
                if (heartCount > 0) {
                    console.log(challenge); // Используем challenge вместо data.challenges[index]
                    console.log(challenge.type);
                    
                    if (challenge.type == "assist") {
                        resetBottomRow();
                        challengeAssist(challenge);
                    }
                    else if (challenge.type == "dialogue") {
                        resetBottomRow();
                        challengeDialogue(challenge);
                    }
                    else if (challenge.type == "selectTranscription") {
                        resetBottomRow();
                        challengeTranscription(challenge);
                    }
                    else if (challenge.type == "select") {
                        resetBottomRow();
                        challengeSelect(challenge);
                    }
                    else if (challenge.type == "readComprehension") {
                        resetBottomRow();
                        challengeReadComprehension(challenge);
                    }
                    else {
                        questionLoad();
                    }
...
let lessonComplete = () => {
    ...
    lottieLesson.style.display = "flex";
    lottieLesson.style.justifyContent = "center";
    lottieLesson.style.height = "400px";
    ...
    img.src = "../assets/svg/leaderboard.svg";
    lottieLesson.appendChild(img);
